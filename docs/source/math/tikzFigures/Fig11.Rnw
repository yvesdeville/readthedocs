...
\begin{document}
    ...
    \begin{tikzpicture}
      ...
      minimum width=4em, minimum height=3em ] at (0, -1.7)
      {$\m{X}_{\texttt{u}}$};
      
      \node[draw=Orange, very thick, fill=orange!20,
      minimum width=1.6em, minimum height=3em ](yu) at (1.6, -1.7)
      {$\m{y}_{\texttt{u}}$};
      
      \node[draw=MonVert, very thick, fill=MonVert!20,
      minimum width=4em, minimum height=4.5em ] at (0, -3.4)
      {$\m{X}_{\texttt{n}}$};

      \node[draw=MonVert, very thick, dashed, fill=MonVert!20,
      minimum width=1.6em, minimum height=4.5em ](yn) at (1.6, -3.4)
      {$\m{y}_{\texttt{n}}$};
      
      \node[draw=MonVert, very thick, fill=MonVert!20,
      minimum width=5em, minimum height=4.5em](ynou) at (3.4, -3.4)
      {$\m{Y}^{\textsf{sim}}_{\texttt{n} \vert \texttt{ou}}$};

      \draw[draw=Orange, very thick] (3.52, -3.52) ellipse
      (0.8em and 0.48em);

      \node(Expl) at (6.5, -1.1) {\parbox{4cm}{
          \small 
          the simulations should become
          conditional on $\m{y}_{\texttt{ou}}$}};
      
      \node (index) at (3.9, -3.52) {};
      \path[->,thick] (yo.east) edge[out=0, in=90](ynou.north);
      \path[->, thick] (yu.east) edge[out=0, in=90](ynou.north);
      
      \path[->, Orange, dashed] (Expl.south) edge[out=270, in=0] (index);
