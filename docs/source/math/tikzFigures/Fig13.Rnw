...
\begin{document}
    ...
    \begin{tikzpicture}
      ...
      {$\m{X}_{\texttt{o}}$};
      \node[draw=MonBleu, very thick, fill=MonBleu!20,
      minimum width=1.6em, minimum height=4.5em ] at (1.6, 0)
      {$\m{y}_{\texttt{o}}$};

      \node[draw=Orange, very thick, fill=orange!20,
      minimum width=4em, minimum height=3em ] at (0, -2.1)
      {$\m{X}_{\texttt{u}}$};

      \node[draw=Orange, very thick, fill=orange!20,
      minimum width=1.6em, minimum height=3em ](yu) at (1.6, -2.1)
      {$\m{y}_{\texttt{u}}$};

      \node[draw=Orange, very thick, fill=orange!20,
      minimum width=5em, minimum height=3em] at (3.4, -2.1)
      {$\m{Y}^{\textsf{sim}}_{\texttt{u} \vert \texttt{on}}$};

      %% column j
      \node[draw=Orange, very thick, fill=orange!50,
      minimum width=0.7em, minimum height=3em](yuonj) at (4.1, -2.1)
      {};
     
      
      \node[draw=MonVert, very thick, fill=MonVert!20,
      minimum width=4em, minimum height=4.5em ] at (0, -4.2)
      {$\m{X}_{\texttt{n}}$};
      
      \node[draw=MonVert, very thick, dashed, fill=MonVert!20,
      minimum width=1.6em, minimum height=4.5em ](yn) at (1.6, -4.2)
      {$\m{y}_{\texttt{n}}$};
      
      \node[draw=MonVert, very thick, fill=MonVert!20,
      minimum width=5em, minimum height=4.5em] at (3.4, -4.2)
      {$\m{Y}^{\textsf{sim}}_{\texttt{n} \vert \texttt{ou}}$};

      %% column j
      \node[draw=MonVertF, very thick, fill=MonVert!50,
      minimum width=0.7em, minimum height=4.5em](ynouj) at (4.1, -4.2)
      {};
      
      %% \draw[draw=Orange, very thick] (3.52, -3.52) ellipse
      %% (0.8em and 0.48em);

      \node(Expl) at (7.5, -2.5) {\parbox{5cm}{ \small
          \begin{spacing}{1.2}
            Using the difference
            $\m{y}_{\texttt{u}} - \m{Y}^{\textsf{sim}}_{ \texttt{u} \vert \texttt{on}}[ \,,
            \,j]$ and the \Or{conditional}
            Kriging Weight matrix
            $\widetilde{\m{W}}(\m{X}_{\texttt{n}} \,\vert \, \m{X}_{\texttt{u}})$
            get the difference  $\m{Y}^{\textsf{sim}}_{ \texttt{n} \vert \texttt{ou}}[ \,,
            \,j] - \m{Y}^{\textsf{sim}}_{\texttt{n}\vert \texttt{o}}[ \, ,\,j]$.
          \end{spacing}
        }};
      
    \path[->, thick] (yuonj) edge (ynouj);
    \path[->, thick] (yu.south) edge[out=270, in=90, looseness=0.7]
    (ynouj.north);
