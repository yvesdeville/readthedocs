...
\begin{document}
    ...
    \sf
    \begin{tikzpicture}
      \node at (0, 0) {\includegraphics[width=7.5cm]{..//figures/residualKriging-1.pdf}};
      \node at (0, -4.2) {\includegraphics[width=7.5cm]{..//figures/residualKriging-2.pdf}};
      \node at (5.7, 0) {\parbox{2.4cm}{\small
          Non-conditional simulations  $y_{\texttt{n}}^{[j]}$.}};
      \node at (5.7, -2.1) {\parbox{2.4cm}{\footnotesize \color{MonBleu}
          The points in orange
          show $\m{y}_{\texttt{u}}$, and the green vertical lines show
          the design $\m{X}_{\texttt{n}}$.}};
      \node at (5.7, -4.2) {\parbox{2.4cm}{\small
          Conditional  simulations  $\m{y}_{\texttt{n} \vert \texttt{u}}^{[j]}$}};
    \end{tikzpicture}
\end{document}
