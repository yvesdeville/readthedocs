...
\begin{document}
    ...
    \begin{tikzpicture}
      ...
      \node[draw=Orange, very thick, fill=orange!20,
      minimum width=1.6em, minimum height=4.5em ] (yu) at (1.4, 0)
      {$\m{y}_{\texttt{u}}$};

      \node[draw=Orange, very thick, fill=orange!20,
      minimum width=5em, minimum height=4.5em](YSimu)  at (3.0, 0)
      {$\m{Y}^{\textsf{sim}}_{\texttt{u}}$};

      \node[draw=Orange, very thick, fill=orange!20,
      minimum width=5em, minimum height=4.5em] (Zu) at (5.2, 0)
      {$\m{Z}_{\texttt{u}}$};

      
      \node[draw=MonVert, very thick, fill=MonVert!20,
      minimum width=4em, minimum height=4.5em ] at (0, -3.6)
      {$\m{X}_{\texttt{n}}$};
      
      \node[draw=MonVert, very thick, dashed, fill=MonVert!20,
      minimum width=1.6em, minimum height=4.5em ] at (1.4, -3.6)
      {$\m{y}_{\texttt{n}}$};
      
      \node[draw=MonVert, very thick, fill=MonVert!20,
      minimum width=5em, minimum height=4.5em] (YSimn) at (3.0, -3.6)
      {$\m{Y}^{\textsf{sim}}_{\texttt{n}}$};
      
      \node[draw=MonVert, very thick, fill=MonVert!20,
      minimum width=5em, minimum height=4.5em] (YSimnu) at (5.2, -3.6)
      {$\m{Y}^{\textsf{sim}}_{\texttt{n} \vert \texttt{u}}$};

      \path[->, thick] (yu.south) edge[out=-40, in=270, looseness=1]
      (Zu.south);
      
      \path[->, thick] (YSimu.south)
      edge[out=-30, in=270] (Zu.south);
      
      \path[->, thick] (YSimn.north) edge[out=60, in=90] (YSimnu);
      
      \path[->, thick] (Zu.south) edge
      [transform canvas={xshift=0.5em}]  (YSimnu);
